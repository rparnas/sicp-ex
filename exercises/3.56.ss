...
is to enumerate, in ascending order with no repetitions, all
positive integers with no prime factors other than 2, 3, or
5. One obvious way to do this is to simply test each integer
in turn to see whether it has any factors other than 2, 3,
and 5. But this is very inefficient, since, as the integers
...
* The elements of "(scale-stream S 2)" are also elements of
"S".
...
sources. For this we define a procedure "merge" that
combines two ordered streams into one ordered result stream,
...
#| Answer -- all multiples of 2, 3, and 5 in order |#
