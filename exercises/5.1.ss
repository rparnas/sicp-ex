...
#| Code from book -- 5.2.1 The Machine Model |#
...
#| Code from book -- 5.2.2 The Assembler |#
...
#| Code from book -- 5.2.3 Generating Execution Procedures for Instructions |#
    ...
    (if (operation-exp? condition)
      (let ([condition-proc
        ...
        (lambda ()
          ...
          (advance-pc pc)))
      (error "assemble" "bad test instruction"))))
...
(define (make-primitive-exp exp machine labels)
  (cond ((constant-exp? exp)
        ...
        (else
         (error "assemble" "Unknown expression type" exp))))
...
#| Answer -- diagram on paper, implementation in 5.1 |#

#| Tests -- infrastructure 

NOTE: Create procedures that return machines, rather than machines themselves so
that all machines are created using the latest version of the assembler.
...
(define-test (begin (let ([m (gcd-machine)])
